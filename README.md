...
- [Currawong China USCI Checker](https://currawongweb.com/verify/china-usci-checker/) - Free browser-based check of an 18-character Chinese Unified Social Credit Code (USCI/USCC): length, permitted characters and the GB 32100-2015 check digit. Runs locally with no account, and the code is not sent to a server by this check. A pass confirms format and arithmetic only - it is not a registry lookup and does not show that a company exists, is currently registered, owns a document or is safe to trade with. The page explains what each field of the code means and links to official GSXT registry-search guidance.
